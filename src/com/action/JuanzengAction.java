...
public class JuanzengAction extends ActionSupport
{
	...
	private WupinDAO wupinDAO;

	public WupinDAO getWupinDAO() {
		return wupinDAO;
	}

	public void setWupinDAO(WupinDAO wupinDAO) {
		this.wupinDAO = wupinDAO;
	}

	private Juanzeng juanzeng = new Juanzeng();

	private Wupin wupin=new Wupin();
	...
	public String juanzengWupin()
	{
		String sql="from Wupin where 1=1 ";

		List juanzengList=wupinDAO.getHibernateTemplate().find(sql);
		Map request=(Map)ServletActionContext.getContext().get("request");
		request.put("juanzengList", juanzengList);
		return ActionSupport.SUCCESS;
	}
